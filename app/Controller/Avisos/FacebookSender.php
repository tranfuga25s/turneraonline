...
class FacebookSender extends AppController implements AvisoAppSender {
    ...
    public function enviar( $id_aviso = null ) {
        ...
        $parametros = array(
            'id' => $aviso['Aviso'],
            'created_time' => date('YYYY-MM-DDThh:mmTZD'),
            'from' => Configure::read( 'Facebook.appId' ),
            'to' => array( 'id' => $aviso['Usuario']['facebook_id'] ),
            'message' => View::render( '../Emails/Facebook/'.Inflector::underscore( $demail['Aviso']['template'] ) )
        );
        // Veo de renderizar el mensaje
        $Facebook = new FB();
        $devolucion = $Facebook->api( array( 'method' => '?', 'params' => array( $parametros ) ) );
    }
