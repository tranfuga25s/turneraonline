	...
	$.ajax( { async: true,
		  ...
		  success: function ( datos, textoEstado ) {
			...
			if( ret.estado == true ) {
				...
				$("#dtimensaje").append( ret.mensaje );
				$("#dtincorrecto").dialog({ buttons: { "Ok": function() { $(this).dialog("close"); } } } );
			}
		  },
		  error: function() {
		  	$("#dtimensaje").append( "Error de comunicacion" );
			$("#dtincorrecto").dialog({ buttons: { "Ok": function() { $(this).dialog("close"); } } } );
		  }
	} );
}
...
foreach( $turnos as $turno ) {
	//pr( $turno );
		echo "<tr id=\"turno".$turno['Turno']['id_turno']."\">";
			...
			echo "<td class=\"actions\">".  $this->Html->tag( 'a', 'Cancelar',  array( 'onclick' => 'cancelar('.  $turno['Turno']['id_turno'].')') ). "&nbsp;".
							//$this->Html->tag( 'a', 'Trasladar', array( 'onclick' => 'trasladar(', $turno['Turno']['id_turno'].')' ) );
							$this->Html->tag( 'a', 'Conservar', array( 'onclick' => 'conservar('. $turno['Turno']['id_turno'].')' ) );
...
<div class="actions">
	...
	<ul>
		...
		<li><?php echo $this->Html->link( 'Conservar todos', '/' ); ?></li>
	</ul>
</div>