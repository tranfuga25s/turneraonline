...
class Calendar2Helper extends AppHelper {
     ...
     */
    public $today_day_class = 'success';
    /**
     *
    ...
    function draw( $options = array() ){
        ...
        // Cambio esta div por el contenido en celdas
        ...
        for($x=(0-$options['prev_count']);$x < 0;$x++){
            ...
            $calendar_top_navigation[] = array( 
                $this->Html->tag('span',
                    $this->Html->link( '<<  ' . strftime( '%B',mktime( 0,0,0,$month_year['month'],1,$month_year['year'] ) ),
	                '#'/*$prev_month_link*/,
	                array( 'title' => 'Mes anterior', 'id' => 'previous-month', 'class' => $this->next_prev_month_class,
        ...
        $calendar_top_navigation[] = array(
        $this->Html->tag( 'span', strftime( '%B', mktime( 0, 0, 0, $options['month'], 1, $options['year'] ) ), array( 'id' => 'calendar-month' ) ).' '.
        $this->Html->tag( 'span', strftime( '%Y', mktime( 0, 0, 0, $options['month'], 1, $options['year'] ) ), array( 'id' => 'calendar-year' ) )
        => array( 'colspan' => 3, 'style' => 'text-align: center;' ) );
        ...
        for($x=0;$x<$options['next_count'];$x++){
            ...
            $calendar_top_navigation[] =  array(
                $this->Html->tag( 'span', 
                    $this->Html->link( strftime( '%B', mktime( 0,0,0,$month_year['month'],1,$month_year['year'] ) ). ' >>',
			                           '#',
	                                   array( 'title' => 'Proximo mes', 'id' => 'next-month', 'class' => $this->next_prev_month_class,
	                                   'onclick' => 'cargarCalendario( '.$month_year['month'].', '.$month_year['year'].' )' )
                                     )
        ...
        $calendar_table_header = $this->Html->tableHeaders( $calendar_top_navigation, array(), array( 'style' => 'text-align: center;') );
        $calendar_table_header .= $this->Html->tableHeaders( $days_of_week, array(), array( 'style' => 'text-align: center;') );
            ...
            for($y=0;$y<7;$y++){
                ...
                if( ($cur_day >= 1)  && ($cur_day <= $days_in_month)){
                    ...
                    if($options['show_day_link'] == true){
            			...
            			if( array_key_exists( $cur_day,  $options['events'][$options['year']][intval($options['month'])] ) ) {
            				$display_day_value = $this->Html->link( $cur_day.'<br />'.
            				                                        '<span class="badge badge-success">'.
            				                                        $options['events'][$options['year']][intval($options['month'])][$cur_day].
            				                                        ' disponibles</span>', 
            				                                        '#'/*$options['link_template']*/, 
            				                                        array( 'onClick' => 'enviarDia( '.$options['year'].', '.$options['month'].', '.$cur_day.' )',
            				                                        'class' => 'info', 'escape' => false ) );
            			} else {
            				$display_day_value = $cur_day /*. '<br /><span class="badge  badge-inverse">0 disponibles</span>'*/;
            			}
		...
		if( array_search( $cur_day,  $options['events'][$options['year']][intval($options['month'])] ) != FALSE ) {
	 	    $td_class_name = '';
                ...
                $cell_content = $this->Html->tag( 'p', $display_day_value );
		...
		if( $td_class_name == '' ) {
			$table_cells[] = array( $cell_content, array( 'class' => $this->day_class ) );
		} else {
        ...
        $calendar_content .= $this->Html->tag('table',$calendar_table_header . $calendar_table_cells, array( 'cellspacing' => 0, 'id' => "tablacalendario", 'class' => 'table table-bordered table-hover' ) );
